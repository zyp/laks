...
def rules(env):
    ...
    with PeriphInstanceModule(env, 'laks.periph.rcc') as m:
        if 'rcc' in periph:
            ...
            env.generated_sources.append(Jinja2(env, 'rcc_enums.cpp', env.laks_path / 'rcc/rcc_enums.cpp.j2', buses = buses))
            m.reexports.append('laks.periph.rcc.enums')

            m.headers.append('rcc/rcc_reg.h')
            ...
            env.laks_sources.append(env.laks_path / 'rcc/rcc_init.cpp')

    with PeriphInstanceModule(env, 'laks.periph.flash') as m:
        if 'stm32_flash' in periph:
            m.headers.append('rcc/stm32_flash.h')
            ...
            env.laks_sources.append(env.laks_path / 'rcc/flash_init.cpp')
